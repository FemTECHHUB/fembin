...
| `Stamp` granularity is uncertain | Values observed were small (1–4) even across thousands of records — possibly a coarser batch counter, not strict per-row versioning. Works correctly in every test so far, but verify with a single-record-edit-and-recheck test before leaning on it harder at scale. **Confirmed 2026-09-04: a live item edit did NOT advance the edited row's `Stamp`.** Product 1613's name was changed in BUSY ("Cable-infinix Micro" -> "Cable-infinix Micro  B"); its `Stamp` stayed at `2`, which the saved checkpoint had already passed — so `WHERE Stamp > last` silently skipped the change, and only a full re-pull saw it. Because of this, products sync is no longer Stamp-gated by default: `catalog_sync_products_strategy` in `config.py` picks `"full"` (every run re-pulls the item master — cheap here, configurable up for bigger installs) vs `"stamp"` (incremental, misses edits) vs `"reconcile"` (incremental + a full re-pull at least every `catalog_sync_products_reconcile_interval_seconds`, driven by the `last_full_at` checkpoint in `sync_state`). Material centers/salesmen still use the Stamp-incremental gate. |
...
| Sales people/salesmen ARE a real BUSY master — don't build a local one | Synced read-only with the exact same bulk-SQL pattern as Material Center (`app/domain/catalog/sync.py`'s `sync_salesmen`). **First got this wrong 2026-08-20**: built a table we owned and let callers create rows into (`sales_people`) before realizing BUSY already has a real master for this — corrected same day (`Salesman` model, read-only sync, dropped the local CRUD/reassign endpoints). Whether BUSY ties this master to a specific Material Center is **unconfirmed** — the generic `Master1` schema shows no such field, so no branch-scoping is applied to the synced list. |
| Which `MasterType` holds "salesmen" is company-specific, not a fixed BUSY constant — **confirmed live 2026-08-20** | Guessed `MasterType=33` ("Executive", BUSY's own generic "Salesmen" label) first — it's real, and the original research-phase probe against this same company found it genuinely empty, which held again on a fresh live re-check the same day. Turned out this business doesn't use Executive at all: a screenshot of BUSY's own "List of Engineer" GUI screen showed 13 real names, which an exact live query (`SELECT Name, Alias FROM Master1 WHERE MasterType=19`) matched **exactly** (all 13 names + aliases, e.g. `Faith` / `Saka Faith`, `Ibrahim` / `Ahmed Ibrahim Oriyomi`). `MasterType=19` is generically "Broker" (`docs/reference/03-constants.md`) — **this company has relabeled it "Engineer"**, its repair-shop equivalent of a sales rep. `sync_salesmen` now queries `MasterType=19`. This mapping is this company's own configuration, not a BUSY-wide fact — a different BUSY install could use 33, 19, or something else entirely for the same concept; re-verify (name+alias GUI cross-check, same technique) before assuming either number elsewhere. |
